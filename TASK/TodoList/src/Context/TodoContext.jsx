...
import { createContext, useContext, useState, useEffect } from 'react';
import { fetchTodos, createTodo, updateTodo, deleteTodoById } from './sanityClient';
...
export const TodoProvider = ({ children }) => {
    ...
    const interval = setInterval(() => {
      ...
      todos.forEach((todo) => {
        if (todo.reminderAt && !todo.notified && now >= todo.reminderAt) {
          ...
          // Update notified status in Sanity
          updateTodo(todo._id, { notified: true });
          
          setTodos((prev) =>
            prev.map((t) =>
              t._id === todo._id ? { ...t, notified: true } : t
            )
          );
        }
      });
  ...
  // Fetch todos from Sanity on mount
  useEffect(() => {
  ...
  const addTodo = async (todoData) => {
    try {
      ...
      const sanitizedData = {
        text: sanitizeText(todoData.text),
        description: todoData.description ? sanitizeText(todoData.description) : '',
